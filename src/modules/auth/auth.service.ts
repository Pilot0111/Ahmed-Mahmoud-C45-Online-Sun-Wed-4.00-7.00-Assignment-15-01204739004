...
class AuthService {
  ...
  signUp = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await this._userModel.checkUser(normalizedEmail);
  ...
  resendOtp = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = await this._userModel.findOne({ filter: { email: normalizedEmail } });
  ...
  confirmEmail = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const otpValue = await this._redisService.get({
      key: this._redisService.generateOtpKey({ email: normalizedEmail }),
    });
    ...
    await this._redisService.deleteKey(
      this._redisService.generateOtpKey({ email: normalizedEmail }),
    );
    await this._redisService.deleteKey(
      this._redisService.maxOtpTriesKey(normalizedEmail),
    );
  ...
  signIn = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const isBlocked = await this._redisService.ttl(
      this._redisService.blockKeyLogin(normalizedEmail),
    );
    ...
    if (!match) {
      await this._redisService.increment(
        this._redisService.maxLoginTriesKey(normalizedEmail),
      );
      const failed_tries = await this._redisService.get({
        key: this._redisService.maxLoginTriesKey(normalizedEmail),
      });

      if (failed_tries >= 5) {
        await this._redisService.setValue({
          key: this._redisService.blockKeyLogin(normalizedEmail),
          value: "1",
          ttl: 300,
        }); // 5 min block
        await this._redisService.deleteKey(
  ...
  signUpGmail = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { email: googleEmail, email_verified, name, picture } = payload as TokenPayload;
      if (!googleEmail)
        return next(new AppError("Email not found in Google token", 400));

      const finalEmail = googleEmail.toLowerCase();
      let user = await this._userModel.findOne({ filter: { email: finalEmail } });
  ...
  uploadImage = async (req: any, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const s3UploadOptions = {
        file: req.file,
        path: `Users/${req.user._id}/uploads`, 
